...
def sozdat_treeview_s_scrollbar(roditel, columns):
    ramka = ttk.Frame(roditel)
    ramka.pack(fill=BOTH, expand=True)
    scrollbar_y = ttk.Scrollbar(ramka, orient=VERTICAL)
    scrollbar_y.pack(side=RIGHT, fill=Y)
    scrollbar_x = ttk.Scrollbar(ramka, orient=HORIZONTAL)
    scrollbar_x.pack(side=BOTTOM, fill=X)
    derevo = ttk.Treeview(ramka, columns=columns, show="headings", yscrollcommand=scrollbar_y.set, xscrollcommand=scrollbar_x.set)
    derevo.pack(fill=BOTH, expand=True)
    scrollbar_y.config(command=derevo.yview)
    scrollbar_x.config(command=derevo.xview)
    return derevo

def dobavit_tovar(derevo):
    okno_dobavleniya = tk.Toplevel()
    okno_dobavleniya.title("Добавление товара")
    okno_dobavleniya.geometry("400x650")
    okno_dobavleniya.configure(bg="azure2")
    vcmd = (okno_dobavleniya.register(validate_price), '%S')
    nadpisi = ["Артикул:", "Название:", "Название категории:", "Цена:", "Марка авто:", "Модель авто:", "Статус:", "Описание:"]
    polya_vvoda = {}
    for i, nadpis in enumerate(nadpisi):
        tk.Label(okno_dobavleniya, text=nadpis, bg="azure2").pack(pady=5)
        if nadpis == "Цена:":
            pole = tk.Entry(okno_dobavleniya, width=40, validate="key", validatecommand=vcmd)
        elif nadpis == "Статус:":
            pole = ttk.Combobox(okno_dobavleniya, values=["В наличии", "Нет", "Ожидается"], state="readonly", width=38)
            pole.set("В наличии")
        else:
    ...
    def sohranit_tovar():
        ...
        cursor = conn.cursor()
        try:
            cursor.execute("""INSERT INTO Tovari (artikul, Nazvanie, nazvanie_kategorii, cena, marka_avto, model_avto, status, opisanie) VALUES (?, ?, ?, ?, ?, ?, ?, ?)""", (znacheniya[0], znacheniya[1], znacheniya[2], znacheniya[3], znacheniya[4], znacheniya[5], znacheniya[6], znacheniya[7]))
            conn.commit()
...
def redaktirovat_tovar(derevo):
    ...
    okno_redaktirovaniya.geometry("400x650")
    okno_redaktirovaniya.configure(bg="azure2")
    vcmd = (okno_redaktirovaniya.register(validate_price), '%S')
    nadpisi = ["Артикул:", "Название:", "Название категории:", "Цена:", "Марка авто:", "Модель авто:", "Описание:", "Статус:"]
    polya_vvoda = {}
    for i, nadpis in enumerate(nadpisi):
        tk.Label(okno_redaktirovaniya, text=nadpis, bg="azure2").pack(pady=5)
        if nadpis == "Цена:":
            pole = tk.Entry(okno_redaktirovaniya, width=40, validate="key", validatecommand=vcmd)
            pole.insert(0, str(tovar[i + 1]) if tovar[i + 1] else "")
        elif nadpis == "Статус:":
            pole = ttk.Combobox(okno_redaktirovaniya, values=["В наличии", "Нет", "Ожидается"], state="readonly", width=38)
            pole.set(str(tovar[i + 1]) if tovar[i + 1] else "В наличии")
        else:
    ...
    def obnovit_tovar():
        ...
        cursor = conn.cursor()
        try:
            cursor.execute("""UPDATE Tovari SET artikul=?, Nazvanie=?, nazvanie_kategorii=?, cena=?, marka_avto=?, model_avto=?, opisanie=?, status=? WHERE id_tovara=?""", (*znacheniya, id_tovara))
            conn.commit()
...
def obnovit_tovary_dlya_sotrudnika(derevo):
    for element in derevo.get_children():
        derevo.delete(element)
    conn = sqlite3.connect('Davidov2.db')
    cursor = conn.cursor()
    for znacheniya in cursor.execute("""
        SELECT T.id_tovara, T.artikul, T.Nazvanie, K.nazvanie, T.cena, T.marka_avto, T.model_avto, T.status, T.opisanie
        FROM Tovari T
        LEFT JOIN Kategorii K ON T.id_kategorii = K.id_kategorii
    """):
        derevo.insert("", END, values=znacheniya)
    conn.close()
...
def dobavit_v_korzinu(derevo):
    ...
    try:
        ...
        cena_tovara = float(znacheniya[3]) if str(znacheniya[3]).replace('.', '').isdigit() else 0
        status_tovara = znacheniya[7] if len(znacheniya) > 7 else ""
        if cena_tovara <= 0:
            messagebox.showwarning("Внимание", "Некорректная цена товара")
            return
        ...
        messagebox.showerror("Ошибка", f"Не удалось добавить товар в корзину: {str(e)}")
...
def pokazat_korzinu(glavnoe_okno, derevo):
    ...
    stolbtsy = ('id_tovara', 'artikul', 'nazvanie', 'cena', 'kolichestvo', 'summa')
    derevo_korziny = ttk.Treeview(ramka_korziny, columns=stolbtsy, show="headings")
    derevo_korziny.pack(fill=BOTH, expand=True)
    scrollbar_y = ttk.Scrollbar(ramka_korziny, orient=VERTICAL, command=derevo_korziny.yview)
    scrollbar_y.pack(side=RIGHT, fill=Y)
    derevo_korziny.configure(yscrollcommand=scrollbar_y.set)
    scrollbar_x = ttk.Scrollbar(ramka_korziny, orient=HORIZONTAL, command=derevo_korziny.xview)
    scrollbar_x.pack(side=BOTTOM, fill=X)
    derevo_korziny.configure(xscrollcommand=scrollbar_x.set)
    derevo_korziny.heading("id_tovara", text="ID")
    derevo_korziny.heading("artikul", text="Артикул")
    ...
    derevo_korziny.column("id_tovara", width=50, anchor="center")
    derevo_korziny.column("artikul", width=90, anchor="center")
    derevo_korziny.column("nazvanie", width=200)
    derevo_korziny.column("cena", width=90, anchor="center")
    derevo_korziny.column("kolichestvo", width=70, anchor="center")
    derevo_korziny.column("summa", width=100, anchor="center")
    obshchaya_summa = 0
    for id_tovara, element in korzina.items():
        summa = element['cena'] * element['kolichestvo']
        obshchaya_summa += summa
        derevo_korziny.insert("", END, values=(id_tovara, element.get('artikul', ''), element['nazvanie'], f"{element['cena']} Руб", element['kolichestvo'], f"{summa} Руб"))
    ramka_itogo = tk.Frame(okno_korziny, bg="azure2")
    ramka_itogo.pack(fill=tk.X, pady=10)
    tk.Label(ramka_itogo, text=f"ИТОГО: {obshchaya_summa} Руб", font=("Arial", 14, "bold"), fg="red", bg="azure2").pack(pady=5)
    ...
    def udalit_iz_korziny():
        ...
        if id_tovara in korzina:
            ...
            messagebox.showinfo("Успех", f"Товар '{values[2]}' удален из корзины")
...
def oformit_zakaz(okno_korziny, glavnoe_okno, derevo):
    ...
    try:
        for id_tovara, element in korzina.items():
            cursor.execute("SELECT status FROM Tovari WHERE id_tovara = ?", (id_tovara,))
            result = cursor.fetchone()
            if result and result[0].lower() != "в наличии":
                messagebox.showwarning("Внимание", f"Товар '{element['nazvanie']}' больше не в наличии")
                return
    except sqlite3.Error as e:
        messagebox.showerror("Ошибка БД", f"Ошибка проверки товаров: {str(e)}")
        conn.close()
        return
    try:
        for id_tovara, element in korzina.items():
...
def avtorizatsiya():
    ...
    if polzovatel:
        ...
        if rol_tekushego_polzovatelya == "администратор":
            ...
            derevo = sozdat_treeview_s_scrollbar(ramka, columns)
            for col in columns:
                derevo.heading(col, text={"id": "Айди пользователя", "login": "Логин", "password": "Пароль", "role": "Роль"}[col])
                derevo.column(col, width=150, anchor="center")
            ...
            columns = ('id_tovara', 'artikul', 'Nazvanie', 'nazvanie_kategorii', 'cena', 'marka_avto', 'model_avto', 'status', 'opisanie')
            derevo = sozdat_treeview_s_scrollbar(ramka, columns)
            headers = ["Айди товара", "Артикул", "Название", "Название категории", "Цена", "Марка авто", "Модель авто",
                       "Статус", "Описание"]
            for col, header in zip(columns, headers):
                derevo.heading(col, text=header)
                derevo.column(col, width=120, anchor="center" if col != "opisanie" else "w")
            ramka_knopok = tk.Frame(glavnoe_okno, bg="azure2")
            ramka_knopok.pack(fill=tk.X, pady=5)
            tk.Button(ramka_knopok, text="Добавить товар", command=lambda: dobavit_tovar(derevo), bg="DarkGreen",
                      fg="white").pack(side=LEFT, padx=5)
            tk.Button(ramka_knopok, text="Редактировать", command=lambda: redaktirovat_tovar(derevo), bg="dim gray",
                      fg="white").pack(side=LEFT, padx=5)
            tk.Button(ramka_knopok, text="Удалить", command=lambda: udalit_tovar(derevo), bg="Brown2", fg="white").pack(
                side=LEFT, padx=5)
            tk.Button(ramka_knopok, text="Обновить", command=lambda: obnovit_tovary_dlya_sotrudnika(derevo),
                      bg="DarkSlateGrey", fg="white").pack(side=LEFT, padx=5)
            obnovit_tovary_dlya_sotrudnika(derevo)
        elif rol_tekushego_polzovatelya == "покупатель":
            columns = ('id_tovara', 'art', 'Nazvanie', 'cena', 'marka_avto', 'model_avto', 'status', 'opisanie')
            derevo = sozdat_treeview_s_scrollbar(ramka, columns)
            headers = ["ID", "Артикул", "Название", "Цена", "Марка авто", "Модель авто", "Статус", "Описание"]
            for col, header in zip(columns, headers):
                derevo.heading(col, text=header)
            derevo.column("id_tovara", width=50, anchor="center")
            derevo.column("art", width=90, anchor="center")
            derevo.column("Nazvanie", width=160)
            derevo.column("cena", width=100, anchor="center")
            derevo.column("marka_avto", width=100, anchor="center")
            derevo.column("model_avto", width=100, anchor="center")
            derevo.column("status", width=80, anchor="center")
            derevo.column("opisanie", width=250)
            ramka_knopok = tk.Frame(glavnoe_okno, bg="azure2")
            ramka_knopok.pack(fill=tk.X, pady=5)
